...
Jedná se o nejstarší a nejjednodušší uvedený operátor pro nalezení bodových příznaků \cite{moravec1980obstacle}. Je uveden proto, že představuje primitivní ideový základ pro další uvedené operátory. Jeho principem je představa, hledaný příznakový bod by měl vynikat ve všech směrech kolem sebe, tj že by na všechny strany od něj měla být výrazná změna v jasu. Moravcův operátor je pouze detektor, deskriptor ani doporučení stran porovnávání bodů nejsou součástí algoritmu.

\subsection{Algoritmus}

Moravec definuje funkci průměrné změny jasu v okolí bodu ve směru posunu (x,y):
\begin{align}	
E_{x,y} = \sum_{u,v} w_{u,v} \lvert I_{u+x, v+y} - I_{u,v} \rvert^2 , (x,y) \in \{(1,0), (1,1), (0,1), (-1,1)\},
\end{align}
	
kde  $I$ je obraz ve formě jasových bodů, $w$ je váhové okénko (Moravcův operátor používá čtvercové binární), $(u,v)$ je aktuální pozice v obrazu a $(x,y)$ je posun se kterým zrovna pracujeme. Znamená to že provedeme posun všech obrazových bodů danými směry, a odečteme od sebe původní a posunutou variantu. Tím získáme informaci o změně jasu při posunutí pod úhly $\alpha \in \{0, 45, 90, 135\}$ stupňů. Poté každé testované místo (každý pixel) překryjeme čtvercovým okénkem, hodnoty pod ním sečteme a výsledek umocníme na druhou. Pro každé testované místo tedy budeme mít 4 hodnoty čtverce změny v jednotlivých směrech.
Ve zbytku algoritmu se pouze z pro každé místo vybere $min\{E_{x,y}\}$, ve vzniklém obrazu se stanoví určitý práh a body s vyšší hodnotou, než je tento práh jsou označeny jako výsledné příznaky.

Jedná se tedy o nalezení bodů, které mají diskrétní diferenci v daných směrech nejméně takovou, jaká je odmocnina
hodnoty prahu.

\subsection{Využití}
Tento operátor má řadu problémů, zejména:
\begin{enumerate}
...
Dnes se již prakticky nevyužívá, ale stal se základem řady dalších algoritmů.

\section{Harris}
		
Harrisův operátor \cite{harris1988combined} vznikl snahou odstranit nedostatky Moravcova operátoru - pracuje se stejnou představou, ale efektivněji.
...
\end{bmatrix}
\end{align}
potom dostaneme výsledný vztah:
\begin{align}
E_{x,y} = [x, y] M \begin{bmatrix} x \\ y \end{bmatrix}
\end{align}
Tento vztah vyjadřuje velikost gradientu obrazu ve směru (libovolného) vektoru $(x,y)$. Matici $M$ se někdy říká Harrisova matice.
Pokud místo čtvercového okna nyní zvolíme Gaussovské:
\begin{align}
w_{u,v} = e^{-(u^2 + v^2)/2\sigma^2},
\end{align}
zbavíme se prvního problému Moravcova operátoru: zanášení šumu do výpočtu nevhodným ("ostrým") okénkem. Směr a velikost derivací je nyní popsán elipsou ve formě matice M, která respektuje všechny možné úhly natočení a pokud bychom zdrojový obraz rotovali, velikost os této elipsy se nezmění. 
...
\section{FAST}
	...
	\subsection{Algoritmus}
		...
		\begin{enumerate}
			...
			\item Protože algoritmus má tendenci označit jako příznaky mnoho sousedících bodů, je vhodné po nalezení všech příznaků algoritmus potlačení nemaximálních hodnot (ang. Non-maximum supression). Ten zajistí, že se z každé spojené oblasti příznakových bodů vybere jen ten, který má největší nebo nejmenší intenzitu a ostatní se zahodí.
		\end{enumerate}
	...
	\subsection{Využití}
		...
		Všude tam, kde je při detekci příznaků potřeba především rychlost.
		
	 

\section{SIFT}
SIFT je technologie zajišťující vyhledání příznakových bodů v obraze, spolu s výrobou deskriptorů pro jejich zpětnou identifikaci (při dalším nalezení). 

Navazuje tím na předchozí algoritmy typu Harris, ale navíc oproti nim klade důraz na vytvoření invariantních deskriptorů se snadnou rozlišitelností.

Její výhodou je robustnost vzhledem k zašumění obrazu, změnám osvětlení, afinním transformacím příznakových bodů a jejich pohybu v prostoru. Deskriptory se vyznačují velkou rozpoznatelností, tzn. různé body nebudou mít podobný deskriptor.

Ikdyž je pomalejší než např. SURF, je použitelná pro zpracování obrazu v reálném čase.

Lze ji využít nejen k identifikaci bodů pro prostorovou lokalizaci, ale též k robustnímu hledání definovaných objektů v obrazu (objekty jsou reprezentovány množinou charakteristických bodů).

Algoritmus extrakce deskriptorů je uspořádán do kaskádovité struktury  za účelem urychlení výpočtu: Složitější operace jsou umístěny co  nejdále v algoritmu tak, aby běžely až po filtraci, tj na co nejmenším množství dat.
...
\begin{enumerate}
	\item Hledání jasových extrémů v celém obrazu nezávisle na zvětšení, neboli měřítku: Provede se pomocí rozdílů gaussianů (Difference-of-Gaussians, DoG).
		...
		Pracuje se s matematickou konstrukcí Laplaciánu Gaussiánů, což vpodstatě znamená rozdělení obrazu na pyramidu postupně více a více rozostřených verzí (rozostření se provede pomocí Gaussovského jádra - masky se vzrůstajícím rozptylem $\sigma{}$). Laplacián poskytne spojitou alternativu rozdílů mezi jednotlivými úrovněmi tohoto postupného rozostření. Tento postup zajišťuje, že najdeme-li někde v těchto "rozdílech" příznakový bod, nalezneme ho stejně i v jiném obrazu, kde se ten samý příznakový bod bude nacházet větší, nebo menší - prostě v jiném měřítku. V takovém obraze se bude příznak nacházet
		jinde v tomto prostoru rozostření, ale bude vypadat stejně, což umožňuje cílené porovnání nezávisle na měřítku.
		
		Protože výše popsaný postup platí pro spojitý prostor, při práci s diskrétním obrazem se aproximuje vytvořením pyramidy postupně více a více rozostřených vrstev, kdy tuto pyramidu ještě rozdělíme do "oktáv" (octaves), které nemají nic společného s číslem 8, ale označení pravdepodobně vychází z hudebního názvosloví, kdy nejvyšší tón oktávy má oproti nejnižšímu dvojnásobnou frekvenci v Hertzích. Nejvyšší rozostření oktávy v SIFTu by mělo mít oproti nejnižšímu dvojnásobný rozptyl $\sigma$. V další oktávě
		se též oproti předchozí pracuje s dvojnásobně redukovaným rozlišením obrazu (každý druhý řádek, každý druhý sloupec), protože se předpokládá, že dojde jenom k minimální ztrátě informace za cenu citelného snížení výkonových nároků.
		
		Tuto "pyramidu gaussiánů" doporučuje článek konstruovat tak, že se skládá ze
		4 oktáv po 5 rozostřeních (měřítkách, ang. scales), první rozostření se doporučuje $\sigma_0 = 1.6$ a rozdíly jednotlivých rozostření jsou $k=\sqrt{2}: \sigma_2 = k \sigma_1 = k^2 \sigma_0$ atd. Tato pravidla stran konstrukce pyramidy si implementace vykládají zpravidla velmi volně. Jediné, co je zřejmě potřeba dodržet je konstantní $k$ mezi vrstvami.
	...
	\item Lokalizace klíčových oblastí: Na každé "kandidátské" lokaci se provede rozhodnutí o umístění a relativní velikosti oblasti. Klíčové oblasti jsou vybrány na základě měřítek jejich stability
		...
		Každý bod výsledné pyramidy z předchozího bodu se porovná s celkem 26 body
		ve svém okolí: 8 bezpřostředně příléhajícími ve své vrstvě a oknem 3x3, tedy 9 bodů na stejné lokaci ve vrstvě nad a pod. Za kandidáta se bod označí v případě, že má ze všech těchto bodů největší nebo nejmenší intenzitu.
		...
		Pro další zpřesnění lze do funkce $D(\mathbf{x})$ dosadit vypočtený bod $\hat{\mathbf{x}}$. Pokud má funkce v tomto bodě v kterémkoli směru větší hodnotu než 0.5, znamená to, že by se jako střed příznaku měl zvolit spíše
...
Tyto příznaky je potřeba uchovávat v nějaké databázi a implementovat algoritmus
pro její správu.
