...
\begin{itemize}
	...
	\item Binární čtvercové okénko výsledek výpočtu zašumuje
...
\begin{figure}[!ht] 
	\centering
	\includegraphics[width=3.7in]{img/Harris_eigen.png}
...
\label{sec:sift}
	...
	\begin{enumerate}
		...
		\item Výroba deskriptoru oblasti: Na každé dané oblasti jsou vypočteny lokální gradienty. Ty jsou převedeny do formy invariantního k deformaci tvaru obrazu a změnám osvětlení.
			...
			\begin{figure}[!ht] 
				\centering
					\includegraphics[width=4in]{img/sift_kp.png}
				\caption[Extrakce SIFT deskriptoru]{Extrakce SIFT deskriptoru. Zdroj: \footnotemark } 	\label{sift_kp}
			\end{figure}
			
			\footnotetext{ \url{http://www.codeproject.com/KB/recipes/619039/SIFT.JPG} }
...
\section{MSER}
	...
	\begin{enumerate}
		...
		\item Pro každý extremální region jsou definovány oblasti jeho popisu. Jedná se o elipsy opsané konvexnímu obalu (anglicky convex hull) oblasti. První ho přímo obepíná, další mají 1.5x, 2x, a 3x takovou velikost.
...
\label{subsec_adaboost}
	...
	\begin{enumerate}
		...
		\item pro každý dosud nepoužitý příznak $j$ (filtr s konkrétní velikostí na konkrétní pozici ve zkoumaném obraze) se vytvoří slabý klasifikátor $f_j(x)$. Ke každému takovému klasifikátoru náleží jeho chyba na trénovacích datech $\epsilon_j = \sum_{i} w_i \lvert f_j(x_i)-y_i \rvert$.
		\item vybere se klasifikátor $f_t(x)$ s nejmenší chybou $\epsilon_t$
		...
		\begin{align}
		F_T(x) = \begin{cases}
				1 & \text{pro } \sum_{t=1}^{T} \alpha_t f_t(x) \geq \frac{1}{2} \sum_{t=1}^{T} \alpha_t \\
				0 & \text{jinak}
			   \end{cases},
		\end{align}
	...
	\begin{figure}[!ht] 
		...
		\caption[Příklad výsledku detekce pomocí Haar algoritmu.]{Příklad výsledku detekce pomocí Haar algoritmu. V tomto případě byly použity dva nezávislé detektory: jeden na detekci obličejů, druhý pro detekci očí. 
		Zdroj: \protect\footnotemark
		}	
		\label{haar_detected}
	\end{figure} 
	
	\footnotetext{		 \protect\url{http://docs.opencv.org/3.1.0/d7/d8b/tutorial_py_face_detection.html}}
